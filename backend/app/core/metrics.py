...
import math
from app.core.models import Signal, ModuleMetric, PersonMetric
...
def compute_metrics(module_id: str, signals: List[Signal], max_total_score_global: float) -> ModuleMetric:
    ...
    silo_factor = clamp((top1_share - 0.4) / 0.6, 0.0, 1.0)
    bus_risk_factor = clamp((2 - bus_factor) / 2.0, 0.0, 1.0)
    criticality_factor = clamp(total_score / max_total_score_global, 0.0, 1.0) if max_total_score_global > 0 else 0.0
    
    # Adjusted Weights: Silo is king.
    risk_index_raw = 100.0 * (0.6 * silo_factor + 0.25 * bus_risk_factor + 0.15 * criticality_factor)
    
    # Dampening: Linear ramp-up for < 10 signals to avoid noise
    if len(signals) < 10:
        risk_index_raw = risk_index_raw * (len(signals) / 10.0)
        
    risk_index = round(risk_index_raw, 2)
