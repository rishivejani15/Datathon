...
import { fetchFinanceData, parseExcelFile } from '../services/GoogleSheetService';
...
// --- Default Mocks as Fallback ---

const defaultCostPerSprint = [
...
const Finance = () => {
    ...
    // State driven by Google Sheet CSV
    const [financeData, setFinanceData] = useState({
        kpis: {
            avgSprintCost: { value: 50000, trend: 2.5 }, // Default
            budgetUtilization: { value: 92, status: 'On track for Q3' },
            roi: { value: 4.2, topInitiative: 'Dashboard AI' },
            anomalies: { value: 'None', status: 'Spending is stable' }
        },
        insights: [],
        sprintCosts: defaultCostPerSprint,
        costVsValue: defaultCostVsValue,
        budgetBurn: defaultBudgetBurn,
        loading: true
    });
    ...
    useEffect(() => {
        async function loadData() {
            ...
            try {
                const data = await fetchFinanceData('/finance_data.csv');
                if (data) {
                    console.log("Data loaded:", data);
                    setFinanceData(prev => ({
                        ...prev,
                        kpis: {
                            avgSprintCost: data.kpis.avgSprintCost.value ? data.kpis.avgSprintCost : prev.kpis.avgSprintCost,
                            budgetUtilization: data.kpis.budgetUtilization.value ? data.kpis.budgetUtilization : prev.kpis.budgetUtilization,
                            roi: data.kpis.roi.value ? data.kpis.roi : prev.kpis.roi,
                            anomalies: data.kpis.anomalies.value ? data.kpis.anomalies : prev.kpis.anomalies
                        },
                        insights: data.insights.length > 0 ? data.insights : prev.insights,
                        sprintCosts: data.sprintCosts.length > 0 ? data.sprintCosts : prev.sprintCosts,
                        costVsValue: data.costVsValue.length > 0 ? data.costVsValue : prev.costVsValue,
                        budgetBurn: data.budgetBurn.length > 0 ? data.budgetBurn : prev.budgetBurn,
                        loading: false
                    }));
                } else {
                    // Handle failure by disabling loading at least, or maybe showing error
    ...
    const handleExport = () => {
        ...
        const rows = financeData.sprintCosts.map(row => [row.sprint, row.cost, row.budget]);
        ...
        >
            ...
            <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 bg-secondary text-secondary-foreground hover:bg-secondary/80 rounded-md px-4 py-2 text-sm font-medium shadow-sm transition-colors border border-border"
                    >
                    ...
                    <a href="/finance_data.csv" target="_blank" className="text-xs text-blue-500 hover:underline flex items-center gap-1">
                        <Zap className="h-3 w-3" /> View Source Data
                    </a>
                </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card className="relative overflow-hidden">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">${financeData.kpis.avgSprintCost.value.toLocaleString()}</div>
                        <p className="text-xs text-muted-foreground mb-2 text-emerald-500 font-medium flex items-center gap-1">
                            {financeData.kpis.avgSprintCost.trend > 0 ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                            {financeData.kpis.avgSprintCost.trend > 0 ? '+' : ''}{financeData.kpis.avgSprintCost.trend}% from last quarter
                        </p>
                ...
                <Card className="relative overflow-hidden">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{financeData.kpis.budgetUtilization.value}%</div>
                        <p className="text-xs text-muted-foreground mb-2">{financeData.kpis.budgetUtilization.status}</p>
                        <div className="w-full bg-secondary h-1.5 rounded-full mt-2 overflow-hidden mb-2">
                            <div className="bg-blue-500 h-full rounded-full" style={{ width: `${financeData.kpis.budgetUtilization.value}%` }} />
                        </div>
                    </CardContent>
                </Card>
                <Card className="relative overflow-hidden">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{financeData.kpis.roi.value}x</div>
                        <p className="text-xs text-muted-foreground mb-2">Top Initiative: {financeData.kpis.roi.topInitiative}</p>
                ...
                <Card className="relative overflow-hidden">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-amber-600 dark:text-amber-500">{financeData.kpis.anomalies.value}</div>
                        <p className="text-xs text-muted-foreground mb-2">{financeData.kpis.anomalies.status}</p>
                    ...
                    <CardContent className="space-y-4">
                        {financeData.insights.length > 0 ? (
                            financeData.insights.map((insight, index) => (
                                <div key={index} className="flex gap-3 p-3 bg-card/50 rounded-lg border border-border shadow-sm">
                                    <div className="mt-1">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-semibold">{insight.type}: {insight.title}</h4>
                                        <p className="text-xs text-muted-foreground mt-1 leading-snug">
                                            <span className={`font-bold ${insight.type === 'Optimization' ? 'text-emerald-600' : 'text-amber-600'}`}>
                                                {insight.value}
                                            </span> detected. {insight.description}
                                        </p>
                                    </div>
                                </div>
                            ))
                    ...
                    <CardContent>
                        <ResponsiveContainer width="100%" height={250}>
                            <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                                ...
                                <Scatter name="Initiatives" data={financeData.costVsValue} fill="hsl(var(--primary))" shape="circle" />
                            </ScatterChart>
                        </ResponsiveContainer>
                    </CardContent>
                </Card>
            ...
            <motion.div variants={itemVariants} className="grid md:grid-cols-2 gap-6">
                ...
                <Card className="col-span-2 lg:col-span-1">
                    ...
                    <CardContent>
                        <ResponsiveContainer width="100%" height={300}>
                            <AreaChart data={financeData.budgetBurn}>
                                <defs>
                ...
                <Card className="col-span-2 lg:col-span-1">
                    ...
                    <CardContent>
                        <ResponsiveContainer width="100%" height={300}>
                            <ComposedChart data={financeData.sprintCosts}>
                                <CartesianGrid strokeDasharray="3 3" className="stroke-muted/20" />
